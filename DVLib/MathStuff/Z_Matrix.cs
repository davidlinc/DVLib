...
namespace OLD_
{
    ...
    {
        public static Matrix Null = new Matrix(null, 0, 0);
        public bool isNull { get { return nums == null; } }
        public readonly double[,] nums;
        int width;
        int height;
        ...
        {
            if (!a.isNull && !b.isNull && a.height == b.height && a.width == b.width)
            {
                double[,] nums = new double[a.width, a.height];
                for (int i = 0; i < a.width; i++)
                {
                    for (int j = 0; j < b.width; j++)
                    {
                        nums[i, j] = a.nums[i, j] + b.nums[i, j];
                    }
                }
                return new Matrix(nums, a.width, a.height);
            }
            return Null;
        }
        ...
        public Matrix transpose()
        {
            if (!isNull)
            {
                double[,] nums = new double[height, width];
                for (int i = 0; i < width; i++)
                {
                    for (int j = 0; j < height; j++)
                    {
                        nums[j, i] = this.nums[i, j];
                    }
                }
                return new Matrix(nums, height, width);
            }
            return Null;
        }
        //余子式
        public Matrix getM(int x, int y)
        {
            if (width == height)
            {
                ...
                for (int i = 0; i < N; ++i)
                {
                    for (int j = 0; j < N; ++j)
                    {
                        if (i < x && j < y) { temp[i, j] = nums[i, j]; }
                        else if (i < x && j > y) { temp[i, j - 1] = nums[i, j]; }
                        else if (i > x && j < y) { temp[i - 1, j] = nums[i, j]; }
                        else if (i > x && j > y) { temp[i - 1, j - 1] = nums[i, j]; }
                    }
                }
                return new Matrix(temp, N - 1, N - 1);
            }
            return Null;
        ...
        public double getVal()
        {
            if (width == height)
            {
                ...
                for (int i = 0; i < N; ++i)
                {
                    res = res + nums[0, i] * t * (getM(0, i).getVal());
                    t *= -1;
                }
                return res;
            }
        ...
        public static Matrix operator *(double factor, Matrix a)
        {
            if (!a.isNull)
            {
                double[,] nums = new double[a.width, a.height];
        ...
        public static Matrix operator *(Matrix a, Matrix b)
        {
            if (!a.isNull && !b.isNull && a.width == b.height)
            {
                double[,] nums = new double[b.width, a.height];
                ...
                return new Matrix(nums, b.width, a.height);
            }
            return Null;
        }
...
namespace MathBase
{
	...
	{
        public static Plane PanelFit(params Vector3[] vector3s)
		{
            double[] result = new double[4];
            int n = vector3s.Length;
            ...
            Matrix AtxA =AT*A;
            Matrix OPPAxTa = AtxA.clone();
            OPPAxTa.InvertGaussJordan();
            Matrix OPPATAxAT = OPPAxTa * AT;
            Matrix DP = OPPATAxAT * E;
            result[0] = DP[0, 0];
            result[1] = DP[1, 0];
			...
			if (result[2]!=0)
			{
            return new Plane(new Vector3(0, 0, -result[3] / result[2]),new Vector3(result[0], result[1], result[2]));
			}     
            else if(result[1]!=0)
			{
                return new Plane(new Vector3(0,-result[3] / result[1] ,0 ), new Vector3(result[0], result[1], result[2]));
            }
            else if (result[0]!=0)
			{
                return new Plane(new Vector3(-result[3] / result[0], 0, 0), new Vector3(result[0], result[1], result[2]));
    ...
    public  class Vector
	{
        ...
        public double this[int index]
		{
            ...
            set
			{
				...
				{
                    double[] nd = new double[index + 1];
                    Array.Copy(elements,nd, elements.Length);
                    elements = nd;
				}
        ...
        public static implicit operator Vector((double, double,double) a)
        {
            return new Vector(a.Item1, a.Item2,a.Item3);
        }
        public static implicit operator Vector((double, double, double, double) a)
        {
            return new Vector(a.Item1, a.Item2, a.Item3, a.Item4);
        }
        public static implicit operator Vector((double, double, double, double, double) a)
        {
            return new Vector(a.Item1, a.Item2, a.Item3, a.Item4, a.Item5);
        }
        public static implicit operator Vector((double, double, double, double, double, double) a)
        {
            return new Vector(a.Item1, a.Item2, a.Item3, a.Item4, a.Item5, a.Item6);
        }
        public static implicit operator Vector((double, double, double, double, double, double,double) a)
        {
            return new Vector(a.Item1, a.Item2, a.Item3, a.Item4, a.Item5, a.Item6,a.Item7);
        }
        public static implicit operator Vector((double, double, double, double, double, double,double,double) a)
        {
            return new Vector(a.Item1, a.Item2, a.Item3, a.Item4, a.Item5, a.Item6,a.Item7,a.Item8);
        }
        public static implicit operator Vector((double, double, double, double, double, double, double, double, double) a)
        {
            return new Vector(a.Item1, a.Item2, a.Item3, a.Item4, a.Item5, a.Item6, a.Item7, a.Item8,a.Item9);
        }
        public Vector(params double[] doubles)
		{
            elements = (double[])doubles.Clone();
		}
        public int dimention { get { return elements.Length; } }


    }
    public class Matrix:IxmlObject<Matrix>
    {
        ...
        public static implicit operator Matrix((Vector,Vector) v)
        {
            return new Matrix(v.Item1,v.Item2);
        }
        public static implicit operator Matrix((Vector, Vector,Vector) v)
        {
            return new Matrix(v.Item1, v.Item2,v.Item3);
        }
        public static implicit operator Matrix((Vector, Vector, Vector, Vector) v)
        {
            return new Matrix(v.Item1, v.Item2, v.Item3,v.Item4);
        }
        public static implicit operator Matrix((Vector, Vector, Vector, Vector,Vector) v)
        {
            return new Matrix(v.Item1, v.Item2, v.Item3, v.Item4,v.Item5);
        }
        public static implicit operator Matrix((Vector, Vector, Vector, Vector, Vector,Vector) v)
        {
            return new Matrix(v.Item1, v.Item2, v.Item3, v.Item4, v.Item5,v.Item6);
        }
        public static implicit operator Matrix((Vector, Vector, Vector, Vector, Vector, Vector,Vector) v)
        {
            return new Matrix(v.Item1, v.Item2, v.Item3, v.Item4, v.Item5, v.Item6,v.Item7);
        }
        public static implicit operator Matrix((Vector, Vector, Vector, Vector, Vector, Vector, Vector,Vector) v)
        {
            return new Matrix(v.Item1, v.Item2, v.Item3, v.Item4, v.Item5, v.Item6, v.Item7,v.Item8);
        }
        public static implicit operator Matrix((Vector, Vector, Vector, Vector, Vector, Vector, Vector, Vector,Vector) v)
        {
            return new Matrix(v.Item1, v.Item2, v.Item3, v.Item4, v.Item5, v.Item6, v.Item7, v.Item8,v.Item9);
        }
        public Matrix(params Vector[] vectors)
		{
            int w = 1;
            int h = vectors.Length;
        ...
        public Matrix(params double[] vs)
		{
            numRows = vs.Length;
            numColumns = 1;
            elements = (double[])vs.Clone();
         ...
         *	              矩阵各元素的值
         */
        public void SetData(params double[] value)
        {
		...
		public XmlElement writeXml(XmlElement element)
		{
            ...
            element.SetAttribute("Columns", Columns.ToString());
            for(int i=0;i<elements.Length;i++)
			{
                element.SetAttribute("Data_"+i, elements[i].ToString());    
			}
            return element;
        }

		public Matrix readXml(XmlElement element)
		{
            int r = element.GetAttributeInt("Rows");
            int c = element.GetAttributeInt("Columns");
            double[] data = new double[r * c];
            for (int i = 0; i < data.Length; i++)
            {
				data[i]=double.Parse( element.GetAttribute("Data_" + i));
            }
